...
if [[ "$NEEDS_RELOGIN" == true ]]; then
    echo "      !/ New group added - log out and back in after this script"
else
    echo "      OK ($CURRENT_USER already in qemu + libvirt)"
fi
...
echo "[4/6] Checking LIBVIRT_DEFAULT_URI in ~/.zshrc..."
if ! grep -q "LIBVIRT_DEFAULT_URI" "${HOME}/.zshrc" 2>/dev/null; then
    echo 'export LIBVIRT_DEFAULT_URI="qemu:///system"' >> "${HOME}/.zshrc"
    echo "      Added to ~/.zshrc - reload your shell (source ~/.zshrc) after this script"
else
